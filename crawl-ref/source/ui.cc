...
void TextEntry::_render()
{
    ...
    // Scroll to keep the cursor in view, as far to the right as possible
    if (cursor_x < m_hscroll)
        m_hscroll = cursor_x;
    else
        m_hscroll = max(0, cursor_x - m_region.width + 1);
    ...
    // TODO: add padding() to left and right of text entry
    const int text_x = m_region.x - m_hscroll;
    ...
    draw_colour draw(m_fg_colour, m_bg_colour);
    cgotoxy(m_region.x+1, m_region.y+1, GOTO_CRT);
...
#ifndef USE_TILE_LOCAL
void TextEntry::set_colour(COLOURS fg, COLOURS bg)
{
    m_fg_colour = fg;
    m_bg_colour = bg;
    _expose();
...
void UIRoot::recv_ui_state_change(const JsonNode *json)
{
    ...
    if (widget_id->tag != JSON_STRING)
        return;
    auto widget = synced_widgets.at(widget_id->string_);
